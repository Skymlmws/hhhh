...
int16_t  magHold,headFreeModeHold; // [-180;+180] 磁场？
...
#define PIT_LO  (1<<(2*PITCH))//前后
#define PIT_CE  (3<<(2*PITCH))
#define PIT_HI  (2<<(2*PITCH))
#define YAW_LO  (1<<(2*YAW))//左右
#define YAW_CE  (3<<(2*YAW))
#define YAW_HI  (2<<(2*YAW))
#define THR_LO  (1<<(2*THROTTLE))//油门
...
int16_t motor[8];
int16_t servo[8] = {
  1500,1500,1500,1500,1500,1500,1500,1000};
...
//setup函数
void setup() {              /////ZigduinoRadio.begin(11)
 RF.begin(RF_Channel);//////////////////////////////////////////////////////////读取数据？/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void loop () {
  ...
  uint8_t axis,i;
  int16_t error,errorAngle;///误差
  int16_t delta;///误差
  int16_t PTerm = 0,ITerm = 0,DTerm, PTermACC, ITermACC;///PID参数
  static int16_t lastGyro[2] = {
    0,0      };
  static int16_t errorAngleI[2] = {
    0,0      };
#if PID_CONTROLLER == 1                       ////////////////////PID控制器类型判断
  ...
  static int16_t initialThrottleHold;///油门保持
  static uint32_t timestamp_fixated = 0;
  int16_t rc;
  int32_t prop = 0;

  if (currentTime > rcTime ) { // 50Hz/////////////////////////////////////////检查是否进入fialsafe模式
    ...
    for(i=0;i<4;i++) {
      ...
      if(rcData[i] < MAXCHECK) stTmp |= 0x40;      // check for MAX/////////////////////////////////////////////////////////////??????????????Data怎么来的？
    }
    if(stTmp == rcSticks) {
...
#endif
#ifdef MULTIPLE_CONFIGURATION_PROFILES
        if      (rcSticks == THR_LO + YAW_LO + PIT_CE + ROL_LO) i=1;    // ROLL left  -> Profile 1//rol lo左，hi右，ce保持
        else if (rcSticks == THR_LO + YAW_LO + PIT_HI + ROL_CE) i=2;    // PITCH up   -> Profile 2//pit高度上下，hi上，ce保持，lo下
        else if (rcSticks == THR_LO + YAW_LO + PIT_CE + ROL_HI) i=3;    // ROLL right -> Profile 3//thr lo低速，ce中，hi快
        if(i) {
  ...
  computeIMU();/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  // Measure loop rate just afer reading the sensors///读取传感器数据
  currentTime = micros();
...
#if MAG                             ////如果|rcCommand[YAW]|<70   ->遥控传来的yaw信号小于70,而且处于MAG_MODE！=0的话，就进入MAG模式
  ...
  //**** PITCH & ROLL & YAW PID ****////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////PID算法
#if PID_CONTROLLER == 1 // evolved oldschool
  if ( f.HORIZON_MODE ) prop = min(max(abs(rcCommand[PITCH]),abs(rcCommand[ROLL])),512);////////////////////////如果是在horizon模式，让picth和roll的遥控值小于512

  // PITCH & ROLL
  for(axis=0;axis<2;axis++) {
    rc = rcCommand[axis]<<1;
    error = rc - imu.gyroData[axis];/////////////////////////////////// 误差(遥控收到的值 与 传感器值的差)
    errorGyroI[axis]  = constrain(errorGyroI[axis]+error,-16000,+16000);       // WindUp   16 bits is ok here/////////////// 计算误差积分
    if (abs(imu.gyroData[axis])>640) errorGyroI[axis] = 0;////////////////////////////////////////////// 如果传感器误差超过640,忽略误差积分
...
//////////////////
    if (f.ANGLE_MODE || f.HORIZON_MODE) { // axis relying on ACC/////////////////////////////////////////////////////////// 如果开启了自稳模式或HORIZON模式
      ...
      ITerm              = ITermACC + ((ITerm-ITermACC)*prop>>9);/////////////////////////// // I值微调(利用prop,这个prop是利用遥控信号算出来的,可以看代码最开头两行)
      PTerm              = PTermACC + ((PTerm-PTermACC)*prop>>9);/////////////////////////// //P值微调
    }
////////////////
    PTerm -= ((int32_t)imu.gyroData[axis]*dynP8[axis])>>6; // 32 bits is needed for calculation   ////////////// P值减去一个内容 //TODO
/////////////////// 偏差 等于 陀螺仪数据 - 上一次的陀螺仪数据
    ...
    lastGyro[axis] = imu.gyroData[axis];/////////////////// // 更新记录的陀螺仪数据
    DTerm          = delta1[axis]+delta2[axis]+delta;// D值计算(利用前三次循环的D值都参与计算)
