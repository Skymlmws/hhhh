...
/**************************************************************************************/
#if defined(__AVR_ATmega1280__) || defined(__AVR_ATmega1281__) || defined(__AVR_ATmega2560__) || defined(__AVR_ATmega2561__)|| defined(__AVR_ATmega128RFA1__)
  #define MEGA
#endif
...
#define SERVO_RATES      {30,30,100,100,100,100,100,100}
...
#elif defined(QUADP) || defined(QUADX) || defined(Y4)|| defined(VTAIL4)//四轴飞行器
  ...
  #endif
  #if defined(CAMTRIG)
    #define SEC_SERVO_FROM   2 // use servo 2
    #define SEC_SERVO_TO     2
  #endif
...
/**************************   atmega128rfa1  ************************************/
    ...
    #define LEDPIN_TOGGLE              PINB |= 1<<1;     //switch LEDPIN state (digital PIN 13)
    #define LEDPIN_OFF                 PORTB &= ~(1<<1);
    #define LEDPIN_ON                  PORTB |= (1<<1);

  #if !defined(RCAUXPIN10) 
      #define BUZZERPIN_PINMODE          pinMode (10, OUTPUT);
      #if defined PILOTLAMP && NUMBER_MOTOR <5
      //pkj
        #define PL_PIN_ON              PORTB |= (1<<4);    //PORTB |= 4;
        #define PL_PIN_OFF             PORTB &= ~(1<<4);   // PORTB &= ~4;
      #else
    ...
    #define POWERPIN_ON                PORTB |= 1<<3;
    #define POWERPIN_OFF               PORTB &= ~(1<<3); //switch OFF WMP, digital PIN 12
  #else
  ...
  #define PPM_PIN_INTERRUPT          attachInterrupt(4, rxInt, RISING); //pin D5

  #define V_BATPIN                   A0    // Analog PIN 0
  #define PSENSORPIN                 A1    // Analog PIN 1
  
/*
  #if defined(A2_A3_PIN_HEX)
    #define SOFT_PWM_1_PIN_HIGH        PORTF |= 1<<5;    //A2
    #define SOFT_PWM_1_PIN_LOW         PORTF &= ~(1<<5);
    #define SOFT_PWM_2_PIN_HIGH        PORTF |= 1<<4;    //A3
    #define SOFT_PWM_2_PIN_LOW         PORTF &= ~(1<<4);
  #else
    #define SOFT_PWM_1_PIN_HIGH        PORTE |= 1<<3;    //D4
    #define SOFT_PWM_1_PIN_LOW         PORTE &= ~(1<<3);
    #define SOFT_PWM_2_PIN_HIGH        PORTE |= 1<<4;    //D5
    #define SOFT_PWM_2_PIN_LOW         PORTE &= ~(1<<4);
  #endif
  #define SOFT_PWM_3_PIN_HIGH        PORTB |= 1<<2;    //D11
  #define SOFT_PWM_3_PIN_LOW         PORTB &= ~(1<<2);
  #define SOFT_PWM_4_PIN_HIGH        PORTB |= 1<<3;    //D12
  #define SOFT_PWM_4_PIN_LOW         PORTB &= ~(1<<3);
*/
//电机
  ...
  #define SERVO_2_PINMODE            pinMode(A3,OUTPUT); // TILT_ROLL  - WING right
  #define SERVO_2_PIN_HIGH           PORTF |= 1<<4;
  #define SERVO_2_PIN_LOW            PORTF &= ~(1<<4);
  #define SERVO_3_PINMODE            pinMode(11,OUTPUT); // CAM TRIG  - alt TILT_PITCH
  #define SERVO_3_PIN_HIGH           PORTB |= 1<<2;
  #define SERVO_3_PIN_LOW            PORTB &= ~(1<<2);
  #if !defined(MONGOOSE1_0)
    #define SERVO_4_PINMODE            pinMode(12,OUTPUT); // new       - alt TILT_ROLL
    #define SERVO_4_PIN_HIGH           PORTB |= 1<<3;
    #define SERVO_4_PIN_LOW            PORTB &= ~(1<<3);
  #endif
...
/**************************************************************************************/
  ...
  //pkj
  #define    PL_CHANNEL OCR2A  //use B since A can be used by camstab
  #define    PL_ISR TIMER2_COMPA_vect
  #define    PL_INIT   TCCR2A=0;TIMSK2|=(1<<OCIE2A);PL_CHANNEL=PL_IDLE;PilotLamp(PL_GRN_OFF);PilotLamp(PL_BLU_OFF);PilotLamp(PL_RED_OFF);PilotLamp(PL_BZR_OFF);
