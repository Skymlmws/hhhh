...
//old                  {9,10,7,8,4,5,A2,A3};      //for a quad+: rear,right,left,front   //+ for y6: 7:under right  8:under left
  uint8_t PWM_PIN[8] = {9,8,7,6,5,4,A2,A3};      //for a quad+: rear,right,left,front   //+ for y6: 7:under right  8:under left
#endif
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void writeMotors() { // [1000;2000] => [125;250]
  /****************  Specific PWM Timers & Registers for the MEGA's   *******************/
        ...
        OCR1A = motor[0]<<3; //  pin D9
      #else
        OCR1A = ((motor[0]<<4) - 16000);
      #endif
    #endif
    #if (NUMBER_MOTOR > 1)
      #ifndef EXT_MOTOR_RANGE 
        OCR1B = motor[1]<<3; //  pin D8
      #else
        OCR1B = ((motor[1]<<4) - 16000);
      #endif
    #endif
    #if (NUMBER_MOTOR > 2)
      #ifndef EXT_MOTOR_RANGE 
        OCR1C = motor[2]<<3; //  pin D7
      #else
        OCR1C = ((motor[2]<<4) - 16000);
      #endif
    #endif
    #if (NUMBER_MOTOR > 3)
      #ifndef EXT_MOTOR_RANGE 
        OCR3C = motor[3]<<3; //  pin D6
      #else
        OCR3C = ((motor[3]<<4) - 16000);
      #endif
    #endif
    #if (NUMBER_MOTOR > 4)
      #ifndef EXT_MOTOR_RANGE 
        OCR3B = motor[4]<<3; //  pin D5
        OCR3A = motor[5]<<3; //  pin D4
      #else
        OCR3B = ((motor[4]<<4) - 16000);
        OCR3A = ((motor[5]<<4) - 16000);
      #endif
    #endif
  #endif
}
...
void initOutput() {                                                                ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  /****************  Specific PWM Timers & Registers for the MEGA's    ******************/
    ...
    #endif
    #if (NUMBER_MOTOR > 1)    //D8
      TCCR1A |= _BV(COM1B1); // connect pin 8 to timer 1 channel B
    #endif
    #if (NUMBER_MOTOR > 2)  //D7
      TCCR1A |= _BV(COM1C1); // connect pin 7 to timer 1 channel C
    #endif
    #if (NUMBER_MOTOR > 3)  //D6
      // init 16bit timer 3
      ...
      TCCR3A |= _BV(COM3C1); // connect pin 6 to timer 3 channel C
    #endif
    #if (NUMBER_MOTOR > 4)    //D5,D4
      
      TCCR3A |= _BV(COM3B1); // connect pin 5 to timer 3 channel B
      TCCR3A |= _BV(COM3A1); // connect pin 4 to timer 3 channel A
    #endif
  #endif
...
//----------
    ...
    #if defined(MEGA) // uses timer 3 Comperator A (11 bit)
...
//----------
    ...
    // init Timer 3, 5 and 4 of the mega for hw PWM
//    TIMSK3 &= ~(1<<OCIE3A); // Disable software PWM  
    #if (PRI_SERVO_TO >= 1) || (SEC_SERVO_TO >= 1)
      TCCR3A |= (1<<WGM31);   // phase correct mode & prescaler to 8 = 1us resolution
      TCCR3A &= ~(1<<WGM30);
      ...
      #if (PRI_SERVO_FROM == 1 || SEC_SERVO_FROM == 1) 
        pinMode(5,OUTPUT);
        TCCR3A |= (1<<COM3B1); // pin D5
      #endif
      #if (PRI_SERVO_FROM <= 2 && PRI_SERVO_TO >= 2) || (SEC_SERVO_FROM <= 2 && SEC_SERVO_TO >= 2) 
        pinMode(4,OUTPUT);
        TCCR3A |= (1<<COM3A1); // pin D4
      #endif
    #endif 
  #endif // mega hw pwm 
}
...
void mixTable() {
  ...
  /****************                   main Mix Table                ******************/
    ...
    motor[3] = PIDMIX(+1,-1,-1); //FRONT_L//X型四轴飞行器
  #elif defined( Y4 )
